...
namespace MeAgendaAi.Service.Services
{
    public class EmployeeWorkHoursService : BaseService<EmployeeWorkHours>, IEmployeeWorkHoursService
    {
        ...
        private readonly IEmployeeWorkHoursRepository _employeeWorkHoursRepository;
        private readonly ISchedulingRepository _schedulingRepository;

        public EmployeeWorkHoursService(IEmployeeWorkHoursRepository employeeWorkHoursRepository,
            ISchedulingRepository schedulingRepository, IEmployeeRepository employeeRepository) : base(
        ...
        public ResponseModel GetAvailableEmployeeWorkHours(DateTime date, Employee employee,
            Domain.Entities.Services service)
        {
            var response = new ResponseModel();
            ...
            try
            {
                ...
                var StartTime = workHours.StartHour;
                var EndTime = workHours.EndHour;
                ...
                intervals.ForEach(interval =>
                {
                    ...
                    if (DateTime.Compare(interval, (DateTime)workHours.StartInterval) >= 0 &&
                        DateTime.Compare(interval, (DateTime)workHours.EndInterval) < 0) exclude.Add(interval);

                    // se estiver dentro fo horário de algum agendamento do dia
                    if (schedulings.Any(x => DateTime.Compare(x.StartTime, interval) <= 0) &&
                        schedulings.Any(x => DateTime.Compare(x.EndTime, interval) > 0))
            ...
            try
            {
                ...
                intervals.ForEach(interval =>
                {
                    if (DateTime.Compare(interval, DateTimeUtil.UtcToBrasilia()) < 0)
                        exclude.Add(interval);
                    else if (DateTime.Compare(interval, (DateTime)workHours.StartInterval) >= 0 &&
                             DateTime.Compare(interval, (DateTime)workHours.EndInterval) < 0)
                        // se estiver dentro do horário de intervalo
                        exclude.Add(interval);
                    else if (schedulings.Any(x => DateTime.Compare(x.StartTime, interval) <= 0) &&
                             schedulings.Any(x => DateTime.Compare(x.EndTime, interval) > 0))
                        // se estiver dentro fo horário de algum agendamento do dia
                        exclude.Add(interval);
                });
        ...
        public ResponseModel GetEmployeeMonthSchedule(string userId, int ano, int mes)
        {
            ...
            try
            {
                if (GuidUtil.IsGuidValid(userId))
                {
                    ...
                    if (employee != null)
                    {
                        var scheduleModel = new List<GetEmployeeWorkScheduleModel>();
                        ...
                        if (workHours != null && workHours.Count > 0)
                            ...
                            {
                                var model = new GetEmployeeWorkScheduleModel
                                {
        ...
        private void RemoveExistentWorkHours(DateModel date, Guid employeeId)
        {
            ...
            if (exists != null) _employeeWorkHoursRepository.Remove(exists);
        }
